...
PORT_LEADER = "/dev/tty.usbmodem58760432681"
PORT_FOLLOWER = "/dev/tty.usbmodem58760430071"

motor_bus_follower = MotorBus(
    port=PORT_FOLLOWER,
    motors={"motor": (1, MODEL)}, # name: (motor_idx, motor_model)
)
motor_bus_follower.connect()


motor_bus_leader = MotorBus(
    port=PORT_LEADER,
    motors={"motor": (1, MODEL)}, # name: (motor_idx, motor_model)
)
motor_bus_leader.connect()
...
def log_control_info(robot: Robot, dt_s, episode_index=None, frame_index=None, fps=None):
    ...
    if not robot.robot_type.startswith("stretch"):
        motor_ids = motor_bus_follower.find_motor_indices(possible_ids=SCAN_IDS)
        ...
        for name in robot.leader_arms:
            ...
            motor_ids = motor_bus_leader.find_motor_indices(possible_ids=SCAN_IDS)
            motor_models = [MODEL for _ in range(len(motor_ids))]
            n_motors = len(motor_ids)
            positions = motor_bus_leader.read_with_motor_ids(
        ...
        for name in robot.follower_arms:
            ...
            if key in robot.logs:
                ...
                motor_models = [MODEL for _ in range(len(motor_ids))]
                n_motors = len(motor_ids)
                positions = motor_bus_follower.read_with_motor_ids(
                    motor_models=motor_models,
                    motor_ids=motor_ids,
                    data_name="Present_Position",
                )
